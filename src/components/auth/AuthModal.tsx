...
export const AuthModal: React.FC = () => {
  ...
  // Initialize Google Identity Services button container when in google-picker mode and valid client ID
  useEffect(() => {
    if (isAuthModalOpen && mode === 'google-picker' && isCustomConfigured) {
      try {
  ...
  }, [isAuthModalOpen, mode, isCustomConfigured, activeClientId]);
  ...
  const triggerGoogleOAuthFlow = async () => {
    ...
    // If client ID is placeholder, directly switch to seamless Google Account selector to avoid Google's 401 error
  ...
  const handleSaveCustomClientId = () => {
    try {
      ...
      if (trimmed) {
        ...
        setSuccessMsg('Google Client ID updated! Please re-open to connect.');
      } else {
      ...
      >
        ...
        {mode === 'google-picker' ? (
          <div className="space-y-4">
            {/* 1-Tap Fast Accounts (Bypasses Google 401 Client Error completely) */}
                    ...
                    return (
                      <div
                        ...
                        className={`p-2.5 rounded-xl border text-left flex items-center justify-between transition-all group cursor-pointer ${
                          isAdmin
                            ? 'bg-gradient-to-r from-rose-950/50 via-purple-950/40 to-amber-950/50 border-rose-400/50 hover:border-rose-400 hover:scale-[1.01]'
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-bold text-white truncate">{acc.name || 'Google User'}</span>
                              {isAdmin && (
                            ...
                            <span className="text-[11px] text-stone-300 font-mono block truncate">{acc.email}</span>
                          </div>
                        </div>
            ...
            {/* Official Google Identity Services Container (Active if real Google Cloud Client ID is configured) */}
            {isCustomConfigured && (
